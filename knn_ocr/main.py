...
def extract_geometric_features(region):
    ...
    return np.array([
        ...
        has_horizontal_lines, hole_area_ratio, has_vertical_lines,
        medium_vertical_lines, kl_feature, aspect_ratio, kls_feature, solidity
    ])
...
def load_and_process_training_data(training_dir):
    ...
    for label_idx, class_folder in enumerate(sorted(training_dir.iterdir())):
...
def classify_and_predict(knn_model, scaler, test_dir, class_names):
    ...
    for img_path in sorted(test_dir.glob("*.png")):
        ...
        for region in regions:
            ...
            region_features = scaler.transform(region_features)
            _, predictions, _, _ = knn_model.findNearest(region_features, k=5)
            bbox = region.bbox
...
if __name__ == "__main__":
    ...
    test_folder = "task/"
    feature_matrix, label_list, scaler = load_and_process_training_data(train_folder)
    knn_classifier = train_knn_classifier(feature_matrix, label_list)
    class_names = [folder.name for folder in sorted(Path(train_folder).iterdir()) if folder.is_dir()]
    classify_and_predict(knn_classifier, scaler, test_folder, class_names)